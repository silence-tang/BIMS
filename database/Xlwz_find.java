...
public class Xlwz_find extends JFrame {
	...
	private String[] name=new String[]{"序号","工号","姓名","车牌号","站点","时间","违章信息"};
	private void refresh(){
		String cc=xlName.getText();
		String sd=startDate.getText();
		String ed=endDate.getText();
		Vector detail=Sql_connetcton.getxl(cc);
		if(detail==null||detail.size()==0){
			JOptionPane.showMessageDialog(getParent(), "线路名错误，请重新输入！", "错误", JOptionPane.ERROR_MESSAGE);
			return;
		}
		//判断日期格式
		...
		name1.setText((String) xlName.getText());
		Vector<Vector> ans=Sql_connetcton.wzFind(2,cc,sd,ed);
	...
	public Xlwz_find() {
		...
		JLabel label_1 = new JLabel("线路名:");
		label_1.setBounds(25, 10, 50, 15);
		contentPane.add(label_1);
		xlName = new JTextField();
		xlName.setBounds(75, 10, 50, 15);
		...
		JLabel label_4 = new JLabel("线路");
		label_4.setBounds(170, 10, 54, 15);
		contentPane.add(label_4);
		
		name1 = new JLabel("");
		name1.setBounds(192, 13, 80, 15);
		contentPane.add(name1);
		xlName.addActionListener(new ActionListener() {
